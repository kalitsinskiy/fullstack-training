...
describe('Exercise 2: Truthiness', () => {
  test('isPrime correctly identifies primes', () => {
    ...
    const notPrimes = [0, 1, 4, 6, 8, 9];

    primes.forEach(val => expect(isPrime(val)).toBeTruthy());
    notPrimes.forEach(val => expect(isPrime(val)).toBeFalsy());
  });
