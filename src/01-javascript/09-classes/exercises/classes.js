...
class Temperature {
  ...
  set celsius(value) {
    if (value < -273.15) {
      throw new RangeError('Celsius temperature cannot be less than -273.15');
    }
  ...
  static fromFahrenheit(fahrenheight){
    return new Temperature((fahrenheight - 32) * 5/9);
  }
}
...
const f = Temperature.fromFahrenheit(212);
...
class Employee extends Person {
  ...
  toString() { return `${super.toString()} - ${this.role}`}
}
...
class Stack {
  ...
  pop() {
    if (this.#currentIndex < 1) {
      throw new RangeError('Out of items');
    }
...
class EventEmitter {
  ...
  on(event, callback){
    const callbacks = this.#registrations.has(event)
      ? this.#registrations.get(event)
      : [];

    this.#registrations.set(event, [...callbacks, callback]);
  }

  off(event, callback) {
    if (!this.#registrations.has(event)) { return; }

    const callbacks = this.#registrations.get(event);
  ...
  emit(event, ...args) {
    ...
    this.#registrations.get(event).forEach((callback) => {
      callback(...args);
    });
  }
}
...
const secondHandler = (data) => console.info('Second handler: ', data);
