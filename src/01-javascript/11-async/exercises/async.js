...
async function sequential() {
  ...
  const result1 = await delay(1000, 'Sequential - First');
  const result2 = await delay(1000, 'Sequential - Second');
  const result3 = await delay(1000, 'Sequential - Third');
...
async function parallel() {
  ...
  const [result1, result2, result3] = await Promise.all([
    delay(1000, 'Parallel - First'),
    delay(1000, 'Parallel - Second'),
    delay(1000, 'Parallel - Third'),
  ]);
