...
// Your code here:
// Explanation:
// .map() is used for arr transform and returns new arr;
// .forEach() is used for side effects, returns anything.
const prices = [10, 20, 30];
const doubled1 = prices.map((price) => price * 2);
...
// Your code here:
const over25User = users.find((user) => user.age > 25);

console.log('First user > 25:', over25User);
...
// Your code here:
const someAbove90 = scores.some((score) => score > 90);
const everyAbove50 = scores.every((score) => score > 50);

console.log('Some score > 90:', someAbove90);
console.log('Every score > 50:', everyAbove50);
...
// Your code here:
const flattenedInfinity = nested.flat(Infinity);
function flattenedRecursice(arr) {
  return arr.reduce((acc, item) => {
    if (Array.isArray(item)) {
      return acc.concat(flattenedRecursice(item));
    }
...
console.log('Flattened Infinity:', flattenedInfinity);
console.log('Flattened Recursive:', flattenedRecursice(nested));
