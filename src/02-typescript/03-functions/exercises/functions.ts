...
// Your function here:
function mergeObjects<T extends object>(...params: T[]): T {
...
// Your code here:
type Validator<T> = (item: T) => { valid: true } | { valid: false; message: string };

const isNonEmpty: Validator<string> = (s) =>
  s.length > 0 ? { valid: true } : { valid: false, message: 'Should be not empty' };

const isPositive: Validator<number> = (n) =>
  n > 0 ? { valid: true } : { valid: false, message: 'Should be positive' };
