...
export default function TodoList() {
  ...
  function addTodo(e: React.SubmitEvent) {
    e.preventDefault();
    if (!newTodo.trim()) return;

    setTodos((prev) => [...prev, { id: nextId, text: newTodo.trim(), completed: false }]);
  ...
  return (
    <div>
      ...
      <ul style={{ maxWidth: '400px', margin: '2rem auto', listStyleType: 'none' }}>
        {todos.map((item) => (
          <li key={item.id} style={{ display: 'flex', margin: '1rem 0' }}>
            <label
