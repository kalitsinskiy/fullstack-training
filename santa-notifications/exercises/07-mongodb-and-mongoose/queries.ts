...
async function main(): Promise<void> {
  ...
  // Your query here:
  const alice = await User.findOne({ firstName: 'Alice' }).select('_id').lean();

  const alicePosts = await Post.find({
    authorId: alice!._id,
    status: 'published',
  })
    .select('title category viewCount -_id')
    .sort({ viewCount: -1 })
    .lean();
  ...
  // Your query here:
  const apiTeaggedPosts = await Post.find({
    status: 'published',
    tags: 'api',
  })
