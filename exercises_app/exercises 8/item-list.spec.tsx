...
describe('ItemList', () => {
  ...
  test('shows loading state on mount', () => {
    server.use(http.get(`${BASE_URL}/api/items`, async () => await new Promise(() => {})));
  ...
  test('clicking Delete sends DELETE with the correct id', async () => {
    ...
    let getCallCount = 0;
    const remainingItems = sampleItems.filter((it) => it.id !== '1');
    ...
    server.use(
      ...
      http.get(`${BASE_URL}/api/items`, () => {
        getCallCount += 1;
        return HttpResponse.json(getCallCount === 1 ? sampleItems : remainingItems);
      }),
    );
    ...
    await waitFor(() => expect(screen.queryByText('Warm socks')).not.toBeInTheDocument());
  });
});
