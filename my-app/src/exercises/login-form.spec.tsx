...
describe('LoginForm', () => {
  // Test 1: Initial render — fields and button present, no alert
  ...
  // Test 2: Empty submit — Zod fires both field errors, onSubmit is never called
  ...
  // Test 3: Bad email, valid password — only the email error appears
  ...
  // Test 4: Valid input — onSubmit called once with the typed credentials
  test('calls onSubmit with { email, password } on valid submit', async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn().mockResolvedValue(undefined);
    render(<LoginForm onSubmit={onSubmit} />);

    await user.type(screen.getByLabelText('Email'), 'taras@shevchenko.ua');
    await user.type(screen.getByLabelText('Password'), 'kobzar1814');
    await user.click(screen.getByRole('button', { name: /sign in/i }));

    await waitFor(() => expect(onSubmit).toHaveBeenCalledTimes(1));
    expect(onSubmit).toHaveBeenCalledWith({ email: 'taras@shevchenko.ua', password: 'kobzar1814' });
  });

  // Test 5: onSubmit rejects — server error message appears in role="alert"
  test('renders server error message when onSubmit rejects', async () => {
    ...
    const serverAlert = alerts.find((el) => el.textContent === 'Invalid credentials');
    expect(serverAlert).toBeInTheDocument();
  });

  // Test 6: After server error, a successful submit clears the error alert
